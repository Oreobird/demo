...
def Batchnorm(name, axes, inputs, is_training=None, stats_iter=None, update_moving_stats=True, fused=True, GPU=True):
    axes_shape = [0, 2, 3]
    if not GPU:
        axes_shape = [0, 1, 2]
    if ((axes == axes_shape) or (axes == [0,2])) and fused==True:
        if axes==[0,2]:
        ...
        if not GPU:
            C_idx = 3
        offset = lib.param(name+'.offset', np.zeros(inputs.get_shape()[C_idx], dtype='float32'))
        scale = lib.param(name+'.scale', np.ones(inputs.get_shape()[C_idx], dtype='float32'))

        moving_mean = lib.param(name+'.moving_mean', np.zeros(inputs.get_shape()[C_idx], dtype='float32'), trainable=False)
        moving_variance = lib.param(name+'.moving_variance', np.ones(inputs.get_shape()[C_idx], dtype='float32'), trainable=False)

        def _fused_batch_norm_training():
            data_format = 'NCHW'
            if not GPU:
                data_format = 'NHWC'
            return tf.nn.fused_batch_norm(inputs, scale, offset, epsilon=1e-5, data_format=data_format)
        def _fused_batch_norm_inference():
            ...
            if not GPU:
                axes_moments = [1, 2]
            mean, var = tf.nn.moments(inputs, axes=axes_moments, keep_dims=True)
