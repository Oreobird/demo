...
def Deconv2D(
    ...
    with tf.name_scope(name) as scope:
        ...
        H_idx = (2 if GPU else 1)
        W_idx = (3 if GPU else 2)
        print("H_idx:{}, W_idx:{}".format(H_idx, W_idx))
        
        try: # tf pre-1.0 (top) vs 1.0 (bottom)
            output_shape = tf.pack([input_shape[0], 2*input_shape[H_idx], 2*input_shape[W_idx], output_dim])
        except Exception as e:
            output_shape = tf.stack([input_shape[0], 2*input_shape[H_idx], 2*input_shape[W_idx], output_dim])
