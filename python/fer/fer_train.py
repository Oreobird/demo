...
MODEL_FILE = "E:/ml/fer/model/fer_binary_model.h5"
...
def predict_img_for_classify(model, img_root):
    imgs = []
    ...
    imgpaths = imgpaths[:len(imgpaths) - len(imgpaths) % BATCH_SIZE]
    ...
    for i in range(len(imgpaths)):
        pred_idx = np.argmax(predictions[i], 0)
        pred_dir = os.path.join(img_root, str(pred_idx))
        if not os.path.exists(pred_dir):
            os.mkdir(pred_dir)
        shutil.move(imgpaths[i], pred_dir + '/' + os.path.basename(imgpaths[i]))
        
    return predictions

#
# predictions = predict_img_for_classify(model, "E:/ml/fer/val/4/")
# predictions = predict_img_for_classify(model, "E:/ml/fer/val/5/")
# predictions = predict_img_for_classify(model, "E:/ml/fer/val/6/")